...
to wrap certain things with HTML elements by using Ruby and Ruby blocks. A picture
(or an example) is worth a thousand words...
...
Probably the most useful (I think) is the @cancel@ method, which automatically provides
a redirect_to :back type of thing.
...
view plugin. If they come up missing, you know why.

The good thing is that I've built this plugin with the ease of extending in mind, so it's rather simple to
